...
public abstract class EDDTableFromAsciiService extends EDDTable{ 
    ...
    protected int responseSubstringStart[] = null;   //[dv]; a value is Integer.MAX_VALUE if not in addAttributes
    protected int responseSubstringEnd[] = null;     //[dv]; a value is Integer.MAX_VALUE if not in addAttributes
    ...
    public static EDDTableFromAsciiService fromXml(Erddap erddap, SimpleXMLReader xmlReader) throws Throwable {
        ...
        if (tDatasetType.equals("EDDTableFromAsciiServiceNOS")) { 
            ...
            throw new Exception("type=\"" + tDatasetType + 
    ...
    public EDDTableFromAsciiService(String tDatasetType, 
        ...
        String errorInMethod = "Error in " + tDatasetType + "(" + 
        ...
        for (int dv = 0; dv < ndv; dv++) {
            ...
            if (resSub.length == 2) {
                ...
                if (i1 < 0   || i1 > 100000 ||
                    i2 <= i1 || i2 > 100000)
                    throw new SimpleException(errorInMethod + "For destinationName=" + tDestName + 
                        ", responseSubstring=\"" + resSubS + "\" is invalid.");
    ...
    /**
     * If your subclass uses the standard system of beforeData, afterData, and noData,
     * you can use this to get the table for the encodedSourceUrl.
     * Custom getDataForDapQuery often use only slight variants of this.
     *
     ...
     */
    protected String findBeforeData(BufferedReader in, 
    ...
    protected String find(BufferedReader in, 
        ...
        throw new SimpleException(
            "Unexpected response from data source (" + error + ")");
    }

    /** 
     * This assumes that the data is ready to be read, starting with s.
     * This doesn't call standardizeResultsTable().
     *
     * @param in  Afterwards, this doesn't call in.close().
     * @param s  This is a partial line (already read from 'in', with the first line of data)
     *    or an empty line (where the next line has the first line of data).
     * @return a table where some of the PrimitiveArrays have data, some don't
     * @throws throwable if trouble
     */
    protected Table getTable(BufferedReader in, String s) throws Throwable {
        ...
        while (s != null) {
            ...
            if (po == 0) {
                //stop now
                //if (reallyVerbose) String2.log("  found afterData=\"" + afterData + "\" at the beginning of a line.");
                break;
            } else if (po > 0) {
                //process this line then stop;
                //if (reallyVerbose) String2.log("  found afterData=\"" + afterData + "\" after the beginning of a line.");
                s = s.substring(0, po);
